...
// blockSpec defines block organization for a given version and error correction level.
...
// blockSpecs indexed by [version-1][levelIndex]
// levelIndex: LevelL=0, LevelM=1, LevelQ=2, LevelH=3
var blockSpecs = [40][4]blockSpec{
	// Version 1
...
func getBlockSpec(version int, level Level) blockSpec {
	col := levelColumn(level)
	if col < 0 || version < 1 || version > 40 {
		return blockSpec{}
	}
	return blockSpecs[version-1][col]
}

func interleave(dataCodewords []byte, version int, level Level) []byte {
	spec := getBlockSpec(version, level)
	totalBlocks := spec.numGroup1Blocks + spec.numGroup2Blocks
	...
	for i := 0; i < totalBlocks; i++ {
		var numData int
		if i < spec.numGroup1Blocks {
			numData = spec.group1DataCodewords
		} else {
			numData = spec.group2DataCodewords
		}
		dataBlocks[i] = dataCodewords[offset : offset+numData]
		offset += numData

		ecBlocks[i] = rsEncode(dataBlocks[i], spec.ecCodewordsPerBlock)
	}

	totalCodewords := 0
	for i := 0; i < totalBlocks; i++ {
		totalCodewords += len(dataBlocks[i]) + len(ecBlocks[i])
	}
	res := make([]byte, 0, totalCodewords)

	maxDataLen := 0
	for i := 0; i < totalBlocks; i++ {
		if len(dataBlocks[i]) > maxDataLen {
			maxDataLen = len(dataBlocks[i])
		}
	}

	for j := 0; j < maxDataLen; j++ {
		for i := 0; i < totalBlocks; i++ {
			if j < len(dataBlocks[i]) {
				res = append(res, dataBlocks[i][j])
			}
		}
	}

	for j := 0; j < spec.ecCodewordsPerBlock; j++ {
		for i := 0; i < totalBlocks; i++ {
			if j < len(ecBlocks[i]) {
				res = append(res, ecBlocks[i][j])
			}
		}
	}
